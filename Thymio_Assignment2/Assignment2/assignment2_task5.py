...
class ThymioController(Node):
    ...
    def __init__(self):
        super().__init__('assignnment2_task5')
        """Initialization."""
    ...
    def update_callback(self):
        ...
        wall_distance = 0.08
        if (not self.collision(wall_distance)):
            self.move(0.15, np.random.uniform(-2, +2))
        elif (self.collision(wall_distance)and (self.obstacle==False)):
            self.get_logger().info("Wall Ahead")
            self.obstacle=True
            self.stop()
        elif(self.obstacle and not self.turned):
            self.stop()
            self.get_logger().info("Finding suitable Path")
            self.rotate(180)
